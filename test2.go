...
func main() {
	mapa, err := separaDatos("11AB398765UJ1A050200N23")
	if err != "" {
		fmt.Println(err)
	} else {
		fmt.Println(mapa)
...
func separaDatos(datoEntrada string) (map[string]string, string) {
	...
	if (larLargo + largoNum + larTipo) < largoTotal {
		var totalLargo int = larLargo + largoNum + larTipo
		for i := 1; i < largoTotal; i++ {
			...
			if (totalLargo + larLargo + largoNum + larTipo) < largoTotal {
				totalLargo = totalLargo + larLargo + largoNum + larTipo
			} else {
				i = largoTotal
			}
		}
...
func rescataValores(largoAcomulado int, datoEntrada string) (string, int, string, string) {
	...
	tipo = datoEntrada[largoAcomulado+larLargo+largoNum : largoAcomulado+larLargo+larTipo+largoNum]
